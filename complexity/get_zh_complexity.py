...
meta_root = '/cpfs01/shared/public/wanjuan_filter_32k-complexity/'
...
path_dict = {domain: os.listdir(data_root + domain) for domain in domain_list}
...
for path_idx, (domain, data_path) in enumerate(path_list):
    ...
    while True:
        ...
        comma_num = content.count(',') + content.count('，')
        space_num = content.count(' ')
        bar_num = content.count('|')

        save_dct[data_dict['id']] = {
...
# srun -p llm_t --ntasks-per-node=64 --ntasks=64 --cpus-per-task=1 python -u complexity/get_zh_complexity.py
